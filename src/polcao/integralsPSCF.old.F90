...
subroutine intgAndOrMom(doINTG,doMOME)
   ...
   !   numAtomSites.
   ! Note that we will do something a bit ugly here. Process 0 will be
   !   responsible for creating, writing to, and closing the HDF5 file only.
   !   All other processes will compute data and send it to process 0. Thus,
   !   the load balancing routine will take modified rank and size data so that
   !   even though process 0 does not participate, all atom-atom interactions
   !   are computed.
   toBalance = (numAtomSites * (numAtomSites + 1))/2
   call MPI_Comm_Size(MPI_COMM_WORLD,mpiSize,mpiErr)
   call MPI_Comm_Rank(MPI_COMM_WORLD,mpiRank,mpiErr)
   call loadBalMPI(toBalance,minAtom,maxAtom,mpiRank-1,mpiSize-1)
