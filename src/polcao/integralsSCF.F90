...
subroutine gaussOverlapOL(ga_coreValeOL,potDim)
...
! Include Global Arrays Fortran headers
!#include "mafdecls.fh"
!#include "global.fh"
...
!   integer :: lazer, beam, pew
!   integer :: howmanyTimes1,howmanyTimes2,howmanyTimes3
!   complex (kind=double), allocatable, dimension (:,:,:)   :: coreCore
!   complex (kind=double), allocatable, dimension (:,:,:)   :: valeVale
!   complex (kind=double), allocatable, dimension (:,:,:)   :: coreVale
...
#endif
   pairXBasisFn2(:,:,:) = 0.0_double

   toBalance = (numAtomSites * (numAtomSites + 1))/2
   call MPI_Comm_size(MPI_COMM_WORLD,mpiSize,mpierr)
   call loadBalMPI(toBalance, minAtom, maxAtom,mpiSize)
...
!      do j = i, numAtomSites
      ...
      call initializeAtomSite(j,2,currentAtomType,currentElements,&
         ...
         & currentPosition,currentAlphas,currentBasisFns)
#ifndef GAMMA
!      allocate(currentPair(currentNumTotalStates(1), &
!            & currentNumTotalStates(2),numKPoints))
!   print *, "allocated currentPair"
!   flush(6)
#else
#endif
...
!   print *, "save current pair"
!   flush(6)
      call saveCurrentPair(i,j,numKPoints,currentPair,ga_valeVale,&
            & ga_coreValeOL,ga_coreCore,currentNumTotalStates)
!   print *, "after save current pair"
!   flush(6)
#else
...
!   call nga_get(ga_valeVale,(/1,1,1/),(/valeDim,valeDim,numKPoints/), &
!     & valeVale, (/valeDim,valeDim/))
!   call nga_get(ga_coreValeOL,(/1,1,1/),(/coreDim,valeDim,numKPoints/), &
!     & coreVale, (/coreDim,valeDim/))
!   call nga_get(ga_coreCore,(/1,1,1/),(/coreDim,coreDim,numKPoints/), &
!     & coreCore, (/coreDim,coreDim/))
!
!   print *, "valeVale"
!   do pew=1,numKPoints
!    do lazer=1,valeDim
!      do beam=1,valeDim
!        print *,lazer,beam,pew
!        print *, valeVale(lazer,beam,pew)
!      enddo
!    enddo
!   enddo
!   print *, "coreVale"
...
!   deallocate(coreCore)
   ...
   call orthoOL(ga_valeVale,ga_coreValeOL,ga_coreCore,ga_valeCore,potDim)

   ! Record the completion of this gaussian integration set.
   call timeStampEnd (8)

   call gaDestroy(ga_coreCore,ga_valeCore,ga_coreVale,&
    & ga_valeVale)
   deallocate(ga_coreCore)
   deallocate(ga_coreVale)
   deallocate(ga_valeVale)

end subroutine gaussOverlapOL

! Two center Kinetic Energy overlap integrals.
subroutine gaussOverlapKE(ga_coreValeOL,potDim)
...
!         oneAlphaPair = 0.0_double
      ...
      call kPointLatticeOriginShift (currentNumTotalStates,currentPair,&
            & latticeVector,numKPoints,0)
      call saveCurrentPair(i,j,numKPoints,currentPair,&
            & ga_valeVale,ga_coreVale,ga_coreCore, &
...
!   enddo ! (Atom loop #2)
   ...
   call ortho(2,ga_valeVale,ga_coreValeOL,ga_coreCore,ga_valeCore,&
     ga_coreVale,potDim, 0,0)
...
subroutine gaussOverlapNP(ga_coreValeOL,potDim)
...
!   enddo ! (Atom loop #2)
   ...
   call ortho (3,ga_valeVale,ga_coreValeOL,ga_coreCore,ga_valeCore,&
     ga_coreVale,potDim, 0,0)

   ! Record the completion of this gaussian integration set.
   call timeStampEnd (10)
  
   call gaDestroy(ga_coreCore,ga_valeCore,ga_coreVale,ga_valeVale)
   deallocate(ga_coreCore)
   deallocate(ga_coreVale)
   deallocate(ga_valeVale)
...
!subroutine gaussOverlapEP(ga_coreValeOL,potDim,anyElecPotInteraction, &
!  & hsize_t_bitsize, currAlphaNumber,currPotElement,currPotNumber, &
!  & currMultiplicity, currPotAlpha, currPotTypeNumber)
subroutine gaussOverlapEP(ga_coreValeOL,potDim, &
  & hsize_t_bitsize, currAlphaNumber,currPotElement,currPotNumber, &
  & currMultiplicity, currPotAlpha, currPotTypeNumber)
...
! Inlcude global arrays headers
#include "mafdecls.fh"
#include "global.fh"

   ! Define the parameters that are necessary to control the computation of
   ! each term in the total electronic potential.
!   integer, intent(inout), dimension(:,:,:) :: anyElecPotInteraction
   integer, intent(in) :: hsize_t_bitsize
!   integer :: elecPotInteraction
...
!            & currentNumTotalStates(2),numKPoints))
      ...
      do k = 1, numCellsReal
         ...
         !   passing this point.
!         if (checkElecPotInteraction(i,j,k,matrixIndex,cellIndex, &
!           & anyElecPotInteraction, hsize_t_bitsize) == 1) then
!            cycle
!         else
            ...
            do while (.true.)
              ...
              !   &(powerOfTwo(currentlmAlphaIndex(alphaIndex(2),2))))
               ...
               call electronicPE(contrib,alphaIndex,currentElements,&
                     & currentlmAlphaIndex,shiftedAtomSiteSep, &
                     & currentPosition,shiftedAtomPos,oneAlphaPair, &
                     & currentAlphas, powerOfTwo, l1l2Switch, &
                     & currAlphaNumber, currPotElement, &
                     & currMultiplicity, currPotNumber, &
                     & currPotAlpha)
...
!   enddo ! (Atom loop #2)
   ...
   call ga_sync()
   call ortho(4,ga_valeVale,ga_coreValeOL,ga_coreCore,ga_valeCore,&
     ga_coreVale,potDim, currPotTypeNumber, currAlphaNumber)
...
subroutine elecPotGaussOverlap(ga_coreValeOL,potDim)
   ...
   integer :: hsize_t_bitsize
   ! anyElecPotInteraction matrix and the number of bits in hsize_t.
!   integer, allocatable, dimension(:,:,:) :: anyElecPotInteraction
   integer :: currPotTypeNumber
   real (kind=double) :: currPotAlpha
   integer :: currPotNumber, currPotElement, currAlphaNumber
   integer :: currMultiplicity
...
!         call gaussOverlapEP(ga_coreValeOL,potDim)
         call ga_sync()
         call gaussOverlapEP(ga_coreValeOL,potDim, &
          & hsize_t_bitsize, currAlphaNumber,currPotElement,currPotNumber, &
          & currMultiplicity, currPotAlpha, currPotTypeNumber)
...
! This subroutine is deprecated. It was causing errors with the 
! parallel implementation and did not save significant amount of time
! for non serial calculations.
function checkElecPotInteraction (i,j,k,matrixIndex,cellIndex, & 
  & anyElecPotInteraction, hsize_t_bitsize)
...
!subroutine electronicPE (contrib,alphaIndex,currentElements,&
!                     & currentlmAlphaIndex,shiftedAtomSiteSep,&
!                     & currentPosition,shiftedAtomPos,oneAlphaPair,&
!                     & currentAlphas)
!subroutine electronicPE(contrib,alphaIndex,currentElements,&
!      & currentlmAlphaIndex,shiftedAtomSiteSep,currentPosition,&
!      & shiftedAtomPos,oneAlphaPair,currentAlphas,powerOfTwo, &
!      & l1l2Switch, currAlphaNumber, currPotElement, currMultiplicity, &
!      & currPotNumber, elecPotInteraction, currPotAlpha)
subroutine electronicPE(contrib,alphaIndex,currentElements,&
      & currentlmAlphaIndex,shiftedAtomSiteSep,currentPosition,&
      & shiftedAtomPos,oneAlphaPair,currentAlphas,powerOfTwo, &
      & l1l2Switch, currAlphaNumber, currPotElement, currMultiplicity, &
   ...
   integer, intent(in), dimension(16) :: powerOfTwo
...
subroutine orthoOL(ga_valeVale,ga_coreValeOL,ga_coreCore,ga_valeCore,potDim)
...
#include "global.fh"
   ...
   integer, intent(inout),dimension(:) :: ga_valeVale,ga_coreValeOL
   integer, intent(inout),dimension(:) :: ga_coreCore
...
!  print *, "before barrier: ", mpiRank
!  flush(6)
!  print *, "after barrier: ", mpiRank
!  flush(6)
!  call ga_print_distribution(ga_coreValeOL)
   do i = 1, numKPoints

      call ga_fill(ga_valeCore,(0.0d0,0.0d0))
      ...
      if (coreDim /= 0) then
         ...
         call ga_sync()
         call valeCoreCoreValeOL (valeDim,coreDim,numKPoints,&
               & ga_valeVale(i), ga_coreValeOL(i),i)
         
         call ga_sync()
         ! Form product of (coreValeOL)(coreCore) in temp matrix (valeCore).
         call coreValeCoreCore (valeDim,coreDim,ga_valeCore,&
               & ga_coreValeOL(i),ga_coreCore(i),i)
         
         call ga_sync()
         ! Finally compute the product of the above
         !   (valeCore)(coreCore) with coreValeOL.
         call makeValeVale (valeDim,coreDim,ga_valeCore,&
               & ga_coreValeOL(i),ga_valeVale(i),i)
#else
         ! Form a product of (valeCore)(coreValeOL) for the overlap
         !   integral (valeVale).
...
subroutine ortho (opCode,ga_valeVale,ga_coreValeOL,ga_coreCore,ga_valeCore,&
   & ga_coreVale,potDim, currPotTypeNumber, currAlphaNumber)
...
#include "global.fh"
   ...
   integer, intent(inout),dimension(:) :: ga_valeVale,ga_coreValeOL
   integer, intent(inout),dimension(:) :: ga_coreVale,ga_coreCore
   integer, intent(inout) :: ga_valeCore
   integer, intent(in) :: potDim
   ...
   if (coreDim /= 0) then
      ...
      do i = 1, numKPoints
        ...
        call ga_sync()
        call valeCoreCoreVale (valeDim,coreDim,ga_valeVale(i),&
              & ga_coreVale(i),ga_coreValeOL(i),i)
      enddo
#else
      do i = 1, numKPoints
