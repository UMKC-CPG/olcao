...
!   call h5pset_shuffle_f (valeVale_plid,hdferr)
   ...
   do i = 1, numKPoints
      ...
      call h5dcreate_f (atomKEOverlap_gid,currentName,H5T_NATIVE_DOUBLE,&
            & valeVale_dsid,atomKEOverlap_gid),hdferr,valeVale_plid)
      ...
      do j = 1, 3
         ...
         call h5dcreate_f (atomDMxyzOL_gid(i),currentName,&
         ...
         call h5dcreate_f (atomMMxyzOL_gid(i),currentName,&
      ...
      do j = 1, potDim
         ...
         call h5dcreate_f(atomPotTermOL_gid(i),currentName,&
   ...
   call h5awrite_f(atomDMxyzOL_aid,H5T_NATIVE_INTEGER,0,attribIntDims,hdferr)
   if (hdferr /= 0) stop 'Failed to initialize atomDMxyzOL_aid'

   call h5awrite_f(atomMMxyzOL_aid,H5T_NATIVE_INTEGER,0,attribIntDims,hdferr)
   if (hdferr /= 0) stop 'Failed to initialize atomMMxyzOL_aid'

   do i = 1, potDim
      call h5awrite_f(atomPotTermOL(i),H5T_NATIVE_INTEGER,0,&
...
subroutine accessSCFIntegralHDF5 (scf_fid)
   ...
   do i = 1, numKPoints
      ...
      do j = 1, 3
         ...
         call h5dopen_f (atomMMKPointOL_gid(i),currentName,&
      ...
      do j = 1, potDim
         write (currentName,fmt="(i7.7)") j
         call h5dopen_f(atomPotTermOL_gid(i),currentName,&
   ...
   call h5aopen_f (atomDMOverlap_gid,'status',atomDMxyzOL_aid,hdferr)
   if (hdferr /= 0) stop 'Failed to open DM overlap aid'

   call h5aopen_f (atomMMOverlap_gid,'status',atomMMxyzOL_aid,hdferr)
