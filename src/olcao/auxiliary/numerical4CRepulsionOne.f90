...
program numerical4CRepulsionOne
...
workPerProcess = meshPoints / mpisize
if ((mod(meshPoints,mpisize) /= 0) .and. (mpirank == 0)) then
   write (6,*) "Make sure that you ask for a number of processors that"
   write (6,*) "divides evenly into",meshPoints,"."
   write (6,*) "This will give you proper load balancing."
   stop
endif
if (mpirank == 0) then
...
beginPoint = mpirank*workPerProcess + 1
endPoint   = beginPoint + workPerProcess - 1

write (6,*) mpirank,beginPoint,endPoint,meshPoints
call flush(6)
!      do i = 1, meshPoints+1
!do i = beginPoint, endPoint
   ! Index guide for coeffs:
   !   Index 1: meshPoints
   !   Index 2: 1-4 for the QN_l values 0-3.
   !   Index 3: 1-4 for centers A,B,C,D.
   !   Index 4: 1-3 for x, y, z
! Iterate through x-coordinate mesh points, apply to sites A,B 
do i = 1+mpirank,meshPoints,mpisize
! [x angular aspect of Site A] * [x angular aspect of Site B]
         prod(1) = coeffs(i,l1(1),1,1)*coeffs(i,l2(1),2,1)
         if (prod(1) == 0.0_double) cycle

!Note change in upper limit from meshPoints+1 to just meshPoints.  Same for
!   other loops too.
! Iterate through x-coordinate mesh points
      do j = 1, meshPoints
...
!               & i,tagOIndex,MPI_COMM_WORLD,mpistatus,mpierr)
          call MPI_RECV(fSumResult,1,MPI_DOUBLE_PRECISION,&
                & i,tagFSum,MPI_COMM_WORLD,mpistatus,mpierr)
...
!         enddo
          ...
          ! Accumulate with rank 0 fSum
          fSum = fSum + fSumResult
      enddo
   else
...
!            & MPI_COMM_WORLD,mpierr)
       call MPI_SEND(fSum,1,MPI_DOUBLE_PRECISION,0,tagFSum,&
             & MPI_COMM_WORLD,mpierr)
   endif

   if (mpirank == 0) then

      fSum = fSum*stepSize**6
      write (6,fmt="(e17.8)") fSum
...
end program numerical4CRepulsionOne
