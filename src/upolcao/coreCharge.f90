...
subroutine makeCoreRho
   ...
   use HDF5
   use O_SCFElecStatHDF5, only: coreChargeDensity_did, &
         & coreChargeDensity_aid, pot
   ...
   !   OLCAO execution.
   call checkAttributeHDF5(coreChargeDensity_aid,"Core charge",hdf5Status)
...
!   care.)
   ...
   if (hdferr /= 0) then
      call stopMPI("Failed to write core charge density")
   endif
