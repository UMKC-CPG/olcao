...
subroutine readKPoints(readUnit, writeUnit)
   ...
   call readData(mpiRank,readUnit,writeUnit,kPointStyleCode,&
         & len('KPOINT_STYLE_CODE'),'KPOINT_STYLE_CODE')

   ! Read the code that defines the type of integration method to be used.
   call readData(mpiRank,readUnit,writeUnit,kPointIntgCode,&
         & len('KPOINT_INTG_CODE'),'KPOINT_INTG_CODE')
   ...
   if (kPointStyleCode == 0) then ! Read an explicit list of kpoints.
      ...
      call readData(mpiRank,readUnit,writeUnit,numKPoints,&
            & len('NUM_BLOCH_VECTORS'),'NUM_BLOCH_VECTORS')
      ...
      call readAndCheckLabel(mpiRank,readUnit,writeUnit,&
            & len('NUM_WEIGHT_KA_KB_KC'),'NUM_WEIGHT_KA_KB_KC')
   ...
   elseif (KPointStyleCode == 1) then
      ...
      call readData(mpiRank,readUnit,writeUnit,3,numAxialKPoints,&
            & len('NUM_KP_A_B_C'),'NUM_KP_A_B_C')

      ! Read the fractional distance that the uniform mesh should be shifted
      !   away from the origin along each a,b,c axis.
      call readData(mpiRank,readUnit,writeUnit,3,kPointShift,&
            & len('KP_SHIFT_A_B_C'),'KP_SHIFT_A_B_C')
   ...
   elseif (KPointStyleCode == 2) then
      ...
      call readData(mpiRank,readUnit,writeUnit,minKPointDensity,&
...
subroutine readSYBDKPoints(readUnit, writeUnit)
   ...
   call readData(mpiRank,readUnit,writeUnit,numPaths,numPathKP,isCartesian,&
   ...
   !   each path.
   call readData(mpiRank,readUnit,writeUnit,numPaths,numHighSymKP(:),0,'')
   ...
   do i = 1, numPaths
      ...
      do j = 1, numHighSymKP(i)
         call readData(mpiRank,readUnit,writeUnit,3,highSymKP(:,j,i),0,'')
      enddo
   enddo
