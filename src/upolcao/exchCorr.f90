...
subroutine makeECMeshAndOverlap
   ...
   !   OLCAO execution.
   call checkAttributeHDF5(exchCorrGroup_aid,"ExchCorr Group",hdf5Status)
...
!write(20,*) "lkmn=", l, k, m
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      write (errorMsg,*) "Failed to write exch corr overlap"
      call stopMPI(errorMsg)
   endif
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      write (errorMsg,*) "Failed to record exchCorrGroup success"
      call stopMPI(errorMsg)
   endif
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      write (errorMsg,*) "Failed to close exchCorrGroup attribute"
      call stopMPI(errorMsg)
   endif
...
subroutine readExchCorrMeshParameters(readUnit, writeUnit)
   ...
   call readData(mpiRank,readUnit,writeUnit,printXCMesh,len('PRINT_XC_MESH'),&
         & 'PRINT_XC_MESH')

   ! Read the number of angular sample vectors.
   call readData(mpiRank,readUnit,writeUnit,numSampleVectors,&
         & len('NUM_ANGULAR_SAMPLE_VECTORS'),'NUM_ANGULAR_SAMPLE_VECTORS')

   ! Read the inner and outer weighting factors.
   call readData(mpiRank,readUnit,writeUnit,2,angSampleWeights(:),&
         & len('WTIN_WTOUT'),'WTIN_WTOUT')

   ! Read the parameters for sampling.
   call readData(mpiRank,readUnit,writeUnit,3,tempData(:),&
...
subroutine getMaxNumRayPoints(numPoints_did,numPoints)
   ...
   call MPI_BCAST(maxNumRayPoints,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
