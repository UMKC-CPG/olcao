...
!write (20,*) "cPV(1)=",dataChunk(1,a,b,c)
      ...
      if ((doXDMFField == 1) .and. (triggerAxis == 1) .and. &
         ...
         if (hdferr /= 0) then
            call stopMPI("Failed to select field hyperslab chunk. a")
         endif
   ...
   enddo ! a-axis loop
      ...
      if ((doXDMFField == 1) .and. (triggerAxis == 2) .and. &
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to select field hyperslab chunk b")
         endif
   ...
   enddo ! b-axis loop
      ...
      if ((doXDMFField == 1) .and. (triggerAxis == 3) .and. &
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to select field hyperslab chunk. c")
         endif
...
if ((doRhoFIELD == 1) .and. (mpiRank == 0)) then
...
!   deallocate (tempImagValeVale)
   ...
   do a = 1, numMeshPoints(1)
      ...
      if ((doXDMFField == 1) .and. (meshTriggerAxis == 1) .and. &
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to select mesh h-slab chunk a")
         endif
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to write mesh_did chunk a")
         endif
   ...
   enddo ! a-axis loop
      ...
      if ((doXDMFField == 1) .and. (meshTriggerAxis == 2) .and. &
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to select hyperslab for mesh b")
         endif
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to write mesh_did chunk b")
         endif
...
!write (6,*) "mDC: ", meshDimsChunk(:)
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to select hyperslab for mesh c")
         endif
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to write mesh_did chunk c")
         endif
      endif
   enddo ! c-axis loop
...
#endif
               if (spin == 2) then
                  write (29+i,fmt="(x,e13.4)",advance="NO") &
                        & profilePsiReal(i,2,j)
#ifndef GAMMA
                  write (29+i,fmt="(x,e13.4)",advance="NO") &
                        & profilePsiImag(i,2,j)
#endif
               endif
            endif
            if (doWavFIELD == 1) then
...
subroutine writeFieldHDF5(did,idx,dataChunk,didName,axis)
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      write(errorMsg,fmt="(4a)") 'Failed to write ',&
            & trim(didName),' chunk ',axis
