   ...
   do h = 1, spin
      ...
      if (mpiRank == 0) then
         if (spin == 1) then
            ...
            write (20,*) "Direct Band Gap(eV)   = ",&
                  & minval(directGap(:,h))*hartree
            call flush (20)
         elseif (h == 1) then
            write (20,*) "(Up) Indirect Band Gap(eV) = ",indirectGap(h)*hartree
            write (20,*) "(Up) Direct Band Gap(eV)   = ",&
                  & minval(directGap(:,h))*hartree
            call flush (20)
         else ! spin == 2 and h == 2
            write (20,*) "(Dn) Indirect Band Gap(eV) = ",indirectGap(h)*hartree
            write (20,*) "(Dn) Direct Band Gap(eV)   = ",&
                  & minval(directGap(:,h))*hartree
            call flush (20)
         endif
      endif
...
!#endif
      ...
      if ((mpiRank == 0) .and. (mod(numKPoints,10) /= 0)) then
         write (20,*)
      endif
...
!         dcCond = dcCond + 
      ...
      if (mpiRank == 0) then
         do i = 1, numEnergyPoints
            ! Write the averaged total, and the x, y, z components, making
            !   sure to convert the energy scale into eV.
