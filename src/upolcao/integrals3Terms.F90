...
subroutine gaussOverlapDM(packedVVDims,did,aid)
   ...
   !   OLCAO execution.
   call checkAttributeHDF5(aid,"Three-center DM overlap",hdf5Status)
...
subroutine gaussOverlapMM(packedVVDims,did,aid)
   ...
   ! Determine if this calculation has already been completed by a previous
   !   OLCAO execution.
   call checkAttributeHDF5(aid,"Three-center MM overlap",hdf5Status)
   if (hdf5Status == 1) then
...
subroutine ortho (opCode,packedVVDims,did,aid)
   ...
   do i = 1, numKPoints
      ...
      do j = 1, 3
         ...
         case (6)
            if (hdferr /= 0) then
               call stopMPI("Failed to write dipole moment vale vale")
            endif
         case (7)
            if (hdferr /= 0) then
               call stopMPI("Failed to write momentum matrix vale vale")
            endif
         end select
      enddo
   enddo ! KPoints
   ...
   case (6)
      if (hdferr /= 0) then
         call stopMPI("Failed to record atom DM overlap success.")
      endif
   case (7)
      if (hdferr /= 0) then
         call stopMPI("Failed to record atom momentum matrix success.")
      endif
   end select
   ...
   case (6)
      if (hdferr /= 0) then
         call stopMPI("Failed to close atom DM overlap attribute.")
      endif
   case (7)
      if (hdferr /= 0) then
         call stopMPI("Failed to close atom momentum matrix attribute.")
      endif
   end select
