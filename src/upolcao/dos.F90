      ...
      if (mpiRank == 0) then
            ...
            do i = 1, numAtomTypes
               ...
               do j = 1, lAngMomCount  ! 1=s; 2=p; 3=d; 4=f
                  do k = 1, atomTypes(i)%numQN_lValeRadialFns(j)
                     ...
                     ! Write the orbital definition in the standard 1s,2s,3s,
                     !   2p,3p notation.  The number is the n quantum number
                     !   and we exclude the orbitals that are in the core and
                     !   begin counting with the orbitals in the valence.
                     !   Note that the s orbitals ascend like 1s, 2s, 3s while
                     !   the p orbitals ascend like 2p, 3p.  The (+j-1)
               ...
               ! Write the raw data for this unit.  The first value is the sum
               !   of all orbitals for this type.  (TDOS for this type.)  The
               !   next values are the QN_nl resolved PDOS for this type.
               do j = 1, numEnergyPoints
            ...
            do i = 1, numAtomSites
               ...
               do j = 1, lAngMomCount
                  do k = 1, atomTypes(currentType)%numQN_lValeRadialFns(j)
                     ...
                     !   accounts for this need.
                     write (69+h,ADVANCE='NO',fmt="(i1,a1,1x)") &
                           & atomTypes(currentType)%numQN_lCoreRadialFns(j) + &
                           & k + j - 1, QN_lLetter(j)
         ...
         elseif (detailCodePDOS == 3) then
            ...
            do i = 1, numAtomSites
               ...
               ! Determine the indices of the pdosComplete to use for this
               !   atom, beginning with the first orbital and ending with
               !   the last for this atom.
               initIndex = cumulNumDOS(i)+1
               finIndex  = cumulNumDOS(i+1)
               ...
               do j = 1, lAngMomCount
                  do k = 1, atomTypes(currentType)%numQN_lValeRadialFns(j)
                     do l = 1, (j-1)*2+1
                        ...
                        !   first number is the n QN (excluding all orbitals
                        !   from the core).  The first letter is the l QN, and
                        !   the string after that is the m QN in xyz notation.
                        !   Note that the s orbitals ascend like 1s, 2s, 3s,
                        !   while the p orbitals ascend like 2p, 3p.  The
                        !   (+j-1) accounts for this need.
                        write (formatString,fmt="(a11,i2.2,a4)") &
                              & "(i1,a1,a1,a", &
               ...
               ! Write the raw data for this unit.  The first value is the TDOS
               !   for this atom.  The other values are the QN_nlm resolved
               !   PDOS for this atom.
               do j = 1, numEnergyPoints
                  write (69+h,ADVANCE="NO",fmt="(e13.6,1x)") &
                        & sum(pdosComplete(initIndex:finIndex,j))
         ...
         ! Compute the weighted average of the energy of a given band across
         !   all kpoints.  This energy value is used as the energy for the
         !   localization index.
         ...
         do i = 1, numKPoints
            ...
            ! Collect weighted sum of the energy for this kpoint over all
            !   states.
            energyValuesAvg(:) = energyValuesAvg(:) + &
                  & energyEigenValues(:numStates,i,h) * kPointWeight(i)
         enddo

         ! Divide the result by two since the sum of kpoint weights is two
         !   and not one.
         energyValuesAvg(:) = energyValuesAvg(:) / 2.0_double
         localizationIndex(:) = localizationIndex(:) / 2.0_double

         ! Record the results to disk as the localization index for each
         !   state, converting the energy scale to eV.
         do i = 1, numStates
            write (79+h,fmt="(1x,f14.8,2x,f14.8)") energyValuesAvg(i) * &
                  & hartree, localizationIndex(i)
         enddo
      endif ! (mpiRank == 0)
   enddo ! (h spin)
