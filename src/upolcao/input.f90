...
subroutine readTitle(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('TITLE'),'TITLE')
...
subroutine readSharedControl(readUnit,writeUnit,inSCF)
   ...
   ! Read the header for this input section.
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('SHARED_INPUT_DATA'),&
         & 'SHARED_INPUT_DATA')


   ! Read the space group number and subnumber.
   call readData(mpiRank,readUnit,writeUnit,spaceGroupNum,spaceGroupSubNum,&
         & len('SPACE_GROUP'),'SPACE_GROUP')


   ! Read the cutoff values for integrals.  (Used by setup and intg.)
   call readData(mpiRank,readUnit,writeUnit,basisFnConvgTemp,electroConvgTemp,&
   ...
   call readData(mpiRank,readUnit,writeUnit,numElectrons,len('NUM_ELECTRONS'),&
      & 'NUM_ELECTRONS')


   ! Read the thermal smearing parameters.
   call readData(mpiRank,readUnit,writeUnit,thermalSigma,maxThermalRange,&
   ...
   call readData(mpiRank,readUnit,writeUnit,fermiSearchLimit,&
...
subroutine readMainControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('MAIN_INPUT_DATA'),&
         & 'MAIN_INPUT_DATA')

   call readData(mpiRank,readUnit,writeUnit,lastIt,len('LAST_ITERATION'),&
         & 'LAST_ITERATION')

   call readData(mpiRank,readUnit,writeUnit,cTest,len('CONVERGENCE_TEST'),&
         & 'CONVERGENCE_TEST')

   call readData(mpiRank,readUnit,writeUnit,xcCodeTemp,len('XC_CODE'),'XC_CODE')

   call readData(mpiRank,readUnit,writeUnit,fbLevel,len('FEEDBACK_LEVEL'),&
         & 'FEEDBACK_LEVEL')

   call readData(mpiRank,readUnit,writeUnit,rlxFact,len('RELAXATION_FACTOR'),&
         & 'RELAXATION_FACTOR')

   call readData(mpiRank,readUnit,writeUnit,iterFlagTDOS,&
         & len('EACH_ITER_FLAGS__TDOS'),'EACH_ITER_FLAGS__TDOS')
   ...
   call readData(mpiRank,readUnit,writeUnit,plusUJFormTemp,numPlusUJItemsTemp,&
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('ID__U__J'),'ID__U__J')
   do i = 1, numPlusUJItemsTemp
      call readData(mpiRank,readUnit,writeUnit,itemID,hubbardU,hundJ,0,'')
   ...
   call readData(mpiRank,readUnit,writeUnit,numSplitTypes,splitAmt,&
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,&
         & len('TYPE_ID__SPIN_SPLIT_FACTOR'),'TYPE_ID__SPIN_SPLIT_FACTOR')
   do i = 1, numSplitTypes
      call readData(mpiRank,readUnit,writeUnit,typeID,splitAmt,0,'')
...
subroutine readDIMOControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('DIPOLE_INPUT_DATA'),&
         & 'DIPOLE_INPUT_DATA')

   ! Read all the DIMO input data.
   call readData(mpiRank,readUnit,writeUnit,3,dipoleCenter,&
         & len('DIPOLE_CENTER'),'DIPOLE_CENTER')
...
subroutine readDOSControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('DOS_INPUT_DATA'),&
         & 'DOS_INPUT_DATA')

   ! Read all the input data.
   call readData(mpiRank,readUnit,writeUnit,deltaDOS,sigmaDOS,0,'')
   call readData(mpiRank,readUnit,writeUnit,eminDOS,emaxDOS,0,'')
   call readData(mpiRank,readUnit,writeUnit,detailCodePDOS,0,'')
...
subroutine readBondControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('BOND_INPUT_DATA'),&
         & 'BOND_INPUT_DATA')

   ! Read the data.
   call readData(mpiRank,readUnit,writeUnit,maxBL,0,'')
   call readData(mpiRank,readUnit,writeUnit,deltaBOND,sigmaBOND,0,'')
   call readData(mpiRank,readUnit,writeUnit,eminBOND,emaxBOND,0,'')
   call readData(mpiRank,readUnit,writeUnit,outputCodeBondQ,0,'')
   call readData(mpiRank,readUnit,writeUnit,doBond3C,0,'')
   call readData(mpiRank,readUnit,writeUnit,maxNumNeighbors,0,'')
...
subroutine readPACSControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('PACS_INPUT_DATA'),&
         & 'PACS_INPUT_DATA')

   call readData(mpiRank,readUnit,writeUnit,excitedAtomPACS,0,'')
   call readData(mpiRank,readUnit,writeUnit,deltaPACS,sigmaPACS,0,'')
   call readData(mpiRank,readUnit,writeUnit,onsetEnergySlackPACS,&
         & energyWindowPACS,0,'')
   call readData(mpiRank,readUnit,writeUnit,numCorePACS,0,'')
   do i = 1, numCorePACS
      call readData(mpiRank,readUnit,writeUnit,5,tempArray,0,'')
...
subroutine readOptcControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('OPTC_INPUT_DATA'),&
         & 'OPTC_INPUT_DATA')
   call readData(mpiRank,readUnit,writeUnit,cutoffEnOPTC,0,'')
   call readData(mpiRank,readUnit,writeUnit,maxTransEnOPTC,0,'')
   call readData(mpiRank,readUnit,writeUnit,deltaOPTC,0,'')
   call readData(mpiRank,readUnit,writeUnit,sigmaOPTC,0,'')
   call readData(mpiRank,readUnit,writeUnit,detailCodePOPTC,0,'')
...
subroutine readNlopControl(readUnit,writeUnit)
   ...
   ! Read input OPTC control data.
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('NLOP_INPUT_DATA'),&
         & 'NLOP_INPUT_DATA')
   call readData(mpiRank,readUnit,writeUnit,cutoffEnNLOP,0,'')
   call readData(mpiRank,readUnit,writeUnit,maxTransEnNLOP,0,'')
   call readData(mpiRank,readUnit,writeUnit,deltaNLOP,0,'')
   call readData(mpiRank,readUnit,writeUnit,sigmaNLOP,0,'')
...
subroutine readSigeControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('SIGE_INPUT_DATA'),&
         & 'SIGE_INPUT_DATA')
   call readData(mpiRank,readUnit,writeUnit,cutoffEnSIGE,0,'')
   call readData(mpiRank,readUnit,writeUnit,maxTransEnSIGE,0,'')
   call readData(mpiRank,readUnit,writeUnit,deltaSIGE,0,'')
   call readData(mpiRank,readUnit,writeUnit,sigmaSIGE,0,'')
...
subroutine readFieldControl(readUnit,writeUnit)
   ...
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('FIELD_INPUT_DATA'),&
         & 'FIELD_INPUT_DATA')
   call readNumMeshPoints(readUnit,writeUnit)
   call readData(mpiRank,readUnit,writeUnit,eminFIELD,emaxFIELD,0,'')
   call readData(mpiRank,readUnit,writeUnit,4,tempArray,0,'')
   call readData(mpiRank,readUnit,writeUnit,doXDMFField,0,'')
   call readData(mpiRank,readUnit,writeUnit,doProfileField,0,'')
...
subroutine readLoEnControl(readUnit,writeUnit)
   ...
   ! Read the input wave function / charge density / potential function
   !   plotting control variables.
   call readAndCheckLabel(mpiRank,readUnit,writeUnit,len('LOEN_INPUT_DATA'),&
         & 'LOEN_INPUT_DATA')
   call readData(mpiRank,readUnit,writeUnit,loenCode,0,'')
   call readData(mpiRank,readUnit,writeUnit,twoj1,twoj2,0,'')
   call readData(mpiRank,readUnit,writeUnit,maxNumNeigh,cutoffLoEn,&
         & angleSqueeze,0,'')
