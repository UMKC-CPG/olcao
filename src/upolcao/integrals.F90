...
subroutine gaussOverlapOL(numComponents,fullCVDims,packedVVDims,did,CVdid,aid)
   ...
   !   OLCAO execution.
   call checkAttributeHDF5(aid,"Two-center overlap",hdf5Status)
...
subroutine gaussOverlapKE(packedVVDims,did,aid)
   ...
   ! Determine if this calculation has already been completed by a previous
   !   OLCAO execution.
   call checkAttributeHDF5(aid,"Two-center KE overlap",hdf5Status)
   if (hdf5Status == 1) then
      call timeStampEnd(9)
      return
   endif
...
subroutine gaussOverlapMV(packedVVDims,did,aid)
   ...
   ! Determine if this calculation has already been completed by a previous
   !   OLCAO execution.
   call checkAttributeHDF5(aid,"Two-center MV overlap",hdf5Status)
   if (hdf5Status == 1) then
...
subroutine gaussOverlapNP(packedVVDims,did,aid)
   ...
   ! Determine if this calculation has already been completed by a previous
   !   OLCAO execution.
   call checkAttributeHDF5(aid,"Three-center NP overlap",hdf5Status)
   if (hdf5Status == 1) then
...
subroutine gaussOverlapHamPSCF(did,aid)
   ...
   ! Determine if this calculation has already been completed by a previous
   !   OLCAO execution.
   call checkAttributeHDF5(aid,"Hamiltonian overlap",hdf5Status)
   if (hdf5Status == 1) then
...
subroutine orthoOL(numComponents,fullCVDims,packedVVDims,did,CVdid,aid)
   ...
   do i = 1, numKPoints
      ...
      if (hdferr /= 0) then
         call stopMPI("Cannot write atom overlap.")
      endif
      ...
      if (coreDim /= 0) then
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Cannot write real atom overlapCV.")
         endif
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Cannot write imaginary atom overlapCV.")
         endif
#else
         if (mpiRank == 0) then
            call h5dwrite_f(CVdid(1,i),H5T_NATIVE_DOUBLE,&
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to record atom overlap success.")
   endif
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to close the atom overlap attribute.")
   endif
...
subroutine ortho (opCode,packedVVDims,did,aid)
   ...
   do i = 1, numKPoints
      ...
      case (2)
         if (hdferr /= 0) then
            call stopMPI("Failed to write kinetic energy vale vale")
         endif
      case (3)
         if (hdferr /= 0) then
            call stopMPI("Failed to write nuclear potential vale vale")
         endif
      case (4)
         if (hdferr /= 0) then
            call stopMPI("Failed to write electronic potential vale vale")
         endif
      case (5) ! Implies that we are doing a scalar rel. calculation.
         if (hdferr /= 0) then
            call stopMPI("Failed to write mass velocity vale vale")
         endif
      end select
   enddo ! numKPoints i
   ...
   case (2)
      if (hdferr /= 0) then
         call stopMPI("Failed to record atom KE overlap success.")
      endif
   case (3)
      if (hdferr /= 0) then
         call stopMPI("Failed to record atom nuclear overlap success.")
      endif
   case (4)
      if (hdferr /= 0) then
         call stopMPI("Failed to record atom pot term overlap success.")
      endif
   case (5)
      if (hdferr /= 0) then
         call stopMPI("Failed to record atom MV overlap success.")
      endif
   end select
   ...
   case (2)
      if (hdferr /= 0) then
         call stopMPI("Failed to close atom KE overlap attribute.")
      endif
   case (3)
      if (hdferr /= 0) then
         call stopMPI("Failed to close atom nuclear overlap attribute.")
      endif
   case (4)
      if (hdferr /= 0) then
         call stopMPI("Failed to close atom pot term overlap attribute.")
      endif
   case (5)
      if (hdferr /= 0) then
         call stopMPI("Failed to close atom MV overlap attribute.")
      endif
   end select
...
subroutine orthoHamPSCF(spin,did,aid)
   ...
   do h = 1, spin
      do i = 1, numKPoints
         ...
         call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
         if (hdferr /= 0) then
            call stopMPI("Failed to write PSCF Hamiltonian vale vale")
         endif
      enddo ! numKPoints i
   enddo ! spin h
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to record atom Ham overlap success.")
   endif
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to close atom Ham overlap attribute.")
   endif
