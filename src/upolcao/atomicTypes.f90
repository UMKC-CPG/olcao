...
subroutine readAtomicTypes(readUnit,writeUnit,inSCF)
   ...
   call readData(mpiRank,readUnit,writeUnit,numAtomTypes,&
         & len('NUM_ATOM_TYPES'),'NUM_ATOM_TYPES')
   ...
   do i = 1, numAtomTypes
      ...
      call readData(mpiRank,readUnit,writeUnit,4,typeIDs,&
      ...
      call readData(mpiRank,readUnit,writeUnit,70,atomTypes(i)%typeLabel,&
      ...
      call readData(mpiRank,readUnit,writeUnit,lAngMomCount,&
      ...
      call readData(mpiRank,readUnit,writeUnit,atomTypes(i)%numOrbAlphas(1),&
            & atomTypes(i)%alphas(:atomTypes(i)%numOrbAlphas(1)),&
            & len('ALPHAS'),'ALPHAS')


      ! Read in the number of core radial basis functions for this atomic type.
      call readData(mpiRank,readUnit,writeUnit,3,tempIntArray(1:3),&
...
subroutine readRadialFns(readUnit,writeUnit,numRadialFns,numOrbAlphas,&
   ...
   if (numRadialFns /= 0) then
      ...
      call readAndCheckLabel(mpiRank,readUnit,writeUnit,&
            & len('NL_RADIAL_FUNCTIONS'),'NL_RADIAL_FUNCTIONS')
      ...
      do j = 1, numRadialFns
         ...
         call readData(mpiRank,readUnit,writeUnit,2,tempIntArray(1:2),0,'')
         ...
         if (tempIntArray(2) <= basisCode) then
            ...
            call readData(mpiRank,readUnit,writeUnit,5,tempIntArray(1:5),0,'')
            ...
            call readData(mpiRank,readUnit,writeUnit,&
                  & numOrbAlphas(QN_lList(i)+1),&
