...
!call flush(20)
   ...
   if (hdferr /= 0) then
      call stopMPI("Failed to read pot alpha overlap.")
   endif
   call MPI_BCAST(potAlphaOverlap(:,:),potDim*potDim,MPI_DOUBLE_PRECISION,0,&
   ...
   if (info /= 0) then
      write (errorMsg, *) 'Fitting dposvx failed. INFO= ', info
   ...
   if (spin == 2) then
      ...
      if (mpiRank == 0) then
         write (20,fmt="(a30,e16.8,a8,e16.8)") &
            & "Spin Difference Matching Error",spinDiffDifference,&
            & " Out Of ", chargeDensityTrace(2)
      endif
...
!generalRho(:potDim,2) = generalRho(:potDim,1)*numElectrons/fittedCharge
   ...
   if (mpiRank == 0) then
      write (20,*) "Constrained fit num e- = ",&
            & dot_product(intgConsts(:potDim),generalRho(:potDim,2))
   endif
   ...
   if (coreDim /= 0) then
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Failed to read core charge density.")
      endif
      call MPI_BCAST(generalRho(:,3),potDim,MPI_DOUBLE_PRECISION,0,&
            & MPI_COMM_WORLD,mpierr)
   else
   ...
   do i = 1, spin
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Cannot write total (or spin up) pot. coefficients.")
      endif
   enddo
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Cannot write total Rho coefficients.")
   endif
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Cannot write vale Rho coefficients.")
   endif

   ! Store the spin difference charge density coefficients if necessary.
   if (spin == 2) then
      if (mpiRank == 0) then
         call h5dwrite_f (spinDiffRhoCoeffs_did(currIteration),&
               & H5T_NATIVE_DOUBLE,generalRho(:,8),terms,hdferr)
      endif
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Cannot write spin difference Rho coefficients.")
      endif
   endif
   ...
   if (currIteration == 1) then
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Cannot write Gaussian exponential alpahs.")
      endif
   endif
...
!call flush(20)
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to read non local neut q pot.")
   endif
   call MPI_BCAST(nonLocalNeutQPot(:,:),potDim*potDim,&
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to read local neut q pot.")
   endif
   call MPI_BCAST(localNeutQPot(:,:),potDim*potDim,&
         & MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,mpierr)
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to read non local nuc q pot.")
   endif
   call MPI_BCAST(elecStatPot(:,4),potDim,&
         & MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,mpierr)
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to read local nuc q pot.")
   endif
   call MPI_BCAST(elecStatPot(:,5),potDim,&
         & MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,mpierr)
   ...
   if (info /= 0) then
      write (errorMsg, *) 'Electrostatics dposvx failed. INFO= ', info
      call stopMPI(errorMsg)
   endif
...
!errorCount = 0
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Failed to read num ray points.")
      endif
      call MPI_BCAST(numRayPoints,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Failed to read radial weights.")
      endif
...
!           enddo
   ...
   call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
   if (hdferr /= 0) then
      call stopMPI("Failed to read exch corr overlap.")
   endif
   call MPI_BCAST(exchCorrOverlap(:,:),potDim*potDim,&
         & MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,mpierr)
   ...
   if (info /= 0) then
      write (errorMsg, *) 'Exchange-correlation dposvx failed. INFO= ', info
      call stopMPI(errorMsg)
   endif
...
!enddo
   ...
   do i = 1, numPotSites
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Failed to read num ray points #2.")
      endif
      call MPI_BCAST(numRayPoints,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Failed to read radial weights #2.")
      endif
      call MPI_BCAST(radialWeight(:numRayPoints),numRayPoints,&
            & MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,mpierr)
      ...
      call MPI_BCAST(hdferr,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
      if (hdferr /= 0) then
         call stopMPI("Failed to read exch rho operator #2.")
      endif
      call MPI_BCAST(exchRhoOp(:,:,:),potDim*maxNumRayPoints*numOpValues,&
            & MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,mpierr)
...
!   usedPotCoeffs(:,:,2)    = jointUsedPotCoeffs(potDim+1:potDim*2,:)
   ...
   if (mpiRank == 0) then
      ...
      !   same potential coefficients and they will have a "SPIN_DN" label. Do
      !   not be confused though, the coefficients for the SPIN_DN section are
      !   not used by a spin non-polarized calculation at all. They are just
      !   cruft that is kept around to maintain file format consistency.
      do i = 1, 2
