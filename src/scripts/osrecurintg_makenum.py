...
def apply_inseperable_substitutions(string):
    ...
    #   is a 1, 2, or 3 and the position of the term with the "2*" factor
    #   can be different in the expanded form of the product term. So, I have
    #   yet to develop a simple regular expression replacement scheme for the
    #   general case and so to move more quickly I'm just going to explicitly
    #   condense each known form. The one trick is that this condensation
    #   must be done *after* the previous condensation of the exponential
    #   terms because they get in the way of this search-and-replace.
    #   Also note, the #3 term has already been partially reduced.
    ...
    string = string.replace("(xyz(1)-B(1))**l2(1)*(xyz(2)-B(2))**l2(2)*"+
            ...
            "2*product((xyz(:)-B(:))**l2(:))")
    string = string.replace("product((xyz(:)-B(:))**l2(:))*(2*xyz(3)-2*B(3))"+
            "/(xyz(3)-B(3))","2*product((xyz(:)-B(:))**l2(:))")
