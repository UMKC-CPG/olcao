...
"""
    ...
    def add_parser_arguments(self, parser):
        ...
        # SCF-only job types (100-series). Each optionally
        #   accepts an edge argument (default "gs").
        jobs.add_argument(
            '-scfdos', dest='job', nargs='?',
            const='scfdos:gs', default=None,
            metavar='EDGE',
            help='SCF + DOS calculation. '
                 'Default SCF basis: FB.'
        )
        jobs.add_argument(
            '-scfbond', dest='job', nargs='?',
            const='scfbond:gs', default=None,
            metavar='EDGE',
            help='SCF + bond order/Q*. '
                 'Default SCF basis: MB.'
        )
        jobs.add_argument(
            '-scfdimo', dest='job', nargs='?',
            const='scfdimo:gs', default=None,
            metavar='EDGE',
            help='SCF + dipole moment. '
                 'Default SCF basis: FB.'
        )
        jobs.add_argument(
            '-scfoptc', dest='job', nargs='?',
            const='scfoptc:gs', default=None,
            metavar='EDGE',
            help='SCF + optical properties. '
                 'Default SCF basis: EB.'
        )
        jobs.add_argument(
            '-scfpacs', dest='job', nargs=1,
            default=None, metavar='EDGE',
            help='SCF + PACS. EDGE is required. '
                 'Default SCF basis: EB.'
        )
        jobs.add_argument(
            '-scfnlop', dest='job', nargs='?',
            const='scfnlop:gs', default=None,
            metavar='EDGE',
            help='SCF + nonlinear optical properties. '
                 'Default SCF basis: EB.'
        )
        jobs.add_argument(
            '-scfsige', dest='job', nargs='?',
            const='scfsige:gs', default=None,
            metavar='EDGE',
            help='SCF + sigma(E). '
                 'Default SCF basis: FB.'
        )
        jobs.add_argument(
            '-scfsybd', dest='job', nargs='?',
            const='scfsybd:gs', default=None,
            metavar='EDGE',
            help='SCF + symmetric band structure. '
                 'Default SCF basis: FB.'
        )
        jobs.add_argument(
            '-scfforce', dest='job', nargs='?',
            const='scfforce:gs', default=None,
            metavar='EDGE',
            help='SCF + forces (experimental). '
                 'Default SCF basis: FB.'
        )
        jobs.add_argument(
            '-scffield', dest='job', nargs='?',
            const='scffield:gs', default=None,
            metavar='EDGE',
            help='SCF + field. '
                 'Default SCF basis: FB.'
        )
        jobs.add_argument(
            '-scfmtop', dest='job', nargs='?',
            const='scfmtop:gs', default=None,
            metavar='EDGE',
            help='SCF + polarization. '
                 'Default SCF basis: FB.'
        )

        # Post-SCF job types (200-series). Each optionally
        #   accepts an edge argument (default "gs").
        jobs.add_argument(
            '-dos', dest='job', nargs='?',
            const='dos:gs', default=None,
            metavar='EDGE',
            help='Post-SCF DOS. '
        ...
        jobs.add_argument(
            '-sybd', dest='job', nargs='?',
            const='sybd:gs', default=None,
            metavar='EDGE',
            help='Post-SCF symmetric band structure. '
                 'Default SCF: FB, post-SCF: FB.'
        )
        jobs.add_argument(
            '-force', dest='job', nargs='?',
            const='force:gs', default=None,
            metavar='EDGE',
            help='Post-SCF forces (experimental). '
                 'Default SCF: FB, post-SCF: FB.'
        )
        jobs.add_argument(
            '-field', dest='job', nargs='?',
            const='field:gs', default=None,
            metavar='EDGE',
            help='Post-SCF field. '
                 'Default SCF: FB, post-SCF: FB.'
        )
    ...
    def reconcile(self, args):
        ...
        subroutine where each job type flag sets appropriate
        defaults for the SCF and post-SCF basis if the user did
        not provide them explicitly via -scf or -pscf."""
        ...
        self.basis_scf = (
            args.scf_basis.lower() if scf_explicit else "fb"
        )
        # String for which pscf basis to use (default none).
        self.basis_pscf = (
            args.pscf_basis.lower() if pscf_explicit else "no"
        )

        # Parse the job specification from argparse.
        if args.job is None:
            # No job type specified: default SCF-only.
            self.job_id = 0
            self.job_name = "scf"
            self.edge = "gs"
        elif isinstance(args.job, list):
            # nargs=1 returns a list (used by -pacs/-scfpacs).
            # Determine job type from which flag was used.
            edge_val = args.job[0]
            # We need to figure out which flag was used.
            # Check the raw sys.argv for the flag name.
            job_type = self._find_job_flag()
            self._apply_job_def(
                job_type, edge_val,
                scf_explicit, pscf_explicit
            )
        elif ':' in args.job:
            # "jobtype:edge" format from the const value.
            job_type, edge_val = args.job.split(':', 1)
            self._apply_job_def(
                job_type, edge_val,
                scf_explicit, pscf_explicit
            )
        else:
            # User supplied an edge argument directly.
            # The job type comes from which flag was activated.
            job_type = self._find_job_flag()
            self._apply_job_def(
                job_type, args.job,
                scf_explicit, pscf_explicit
        ...
        if self.edge not in EDGE_MAP:
            sys.exit(
                ...
                f"Valid edges: {', '.join(EDGE_MAP.keys())}"
            )
        self.qn_n, self.qn_l = EDGE_MAP[self.edge]


    def _find_job_flag(self):
        """Scan sys.argv to find which job-type flag was used.

        This is needed because argparse maps all job-type flags
        to the same 'job' dest, so we cannot tell from args
        alone which flag triggered."""
        job_flags = set(JOB_DEFS.keys())
        for arg in sys.argv[1:]:
            if arg.startswith('-'):
                candidate = arg.lstrip('-').lower()
                if candidate in job_flags:
                    return candidate
        return "scf"


    def _apply_job_def(self, job_type, edge,
                       scf_explicit, pscf_explicit):
        """Apply the job definition for the given job type,
        setting jobID, jobName, edge, and default bases."""
        if job_type not in JOB_DEFS:
            sys.exit(
                f"Error: unknown job type '{job_type}'."
            )
        ...
        # Apply default bases only if the user did not set
